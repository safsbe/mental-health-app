...
function generateISODatesBetween(startDate: string, endDate: string): string[] {
  ...
  for (
    let days = 1;
    days <= differenceInCalendarDays(endDateObj, startDateObj);
    days++
  )
...
export const diaryApi = createApi({
  ...
  tagTypes: ['diaryEntry', 'diaryEntryMoodRating', 'diaryEntrySleepRating'],
  endpoints: build => ({
    ...
    saveDiaryEntry: build.mutation({
      queryFn: async (diaryEntry: ActiveDiaryEntry) => {
        ...
        try {
          await db.insert(DiaryEntries).values(insertValue).onConflictDoUpdate({
    ...
    saveDiaryEntryMoodRating: build.mutation<
      {},
      {moodRating: number; entryDate: string | undefined}
    >({
      queryFn: async ({
        moodRating,
    ...
    getDiaryEntryMoodRating7Days: build.query<
      (number | null)[],
      string | undefined
    >({
      queryFn: async (
        lastEntryDate: string = format(new Date(), 'yyyy-MM-dd'),
      ) => {
        ...
        const entryDates = generateISODatesBetween(
          firstEntryDate,
          lastEntryDate,
        );
          ...
          return {
            data: await db
              ...
              .then(dbReturns =>
                entryDates.map(
                  entryDate =>
                    dbReturns?.find(x => x.entryDate === entryDate)
                      ?.moodRating || null,
                ),
              ),
          };
    ...
    saveDiaryEntrySleepRating: build.mutation<
      {},
      {sleepRating: number; entryDate: string | undefined}
    >({
      queryFn: async ({
        sleepRating,
        entryDate = format(new Date(), 'yyyy-MM-dd'),
    ...
    getDiaryEntrySleepRating7Days: build.query<
      (number | null)[],
      string | undefined
    >({
      queryFn: async (
    ...
    getDiaryEntryExtended: build.query<{}, string>({
      queryFn: async entryDate => {
        try {
          const [diaryEntryResult, diaryMomentResult] =
